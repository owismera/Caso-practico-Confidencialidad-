    ...
    if 'usuario' in st.session_state: # Verificamos si la variable usuario esta en el session state
        ...
        if st.session_state['pagina'] == 'login':
            ...
            with st.form("frmCambiar"):
                ...
                actual_password = st.text_input("Actual Contraseña", type="password")
                nueva_password = st.text_input("Nueva Contraseña", type="password")
                confirmar_password = st.text_input("Confirmar Contraseña", type="password")

                btnGuardar = st.form_submit_button("Guardar") # Botón de guardado
                dfusuarios = pd.read_csv('usuarios.csv')
                usuario = dfusuarios[dfusuarios['usuario']==parUsuario]

                if btnGuardar:
                    if not usuario.empty:  # Verificamos que el usuario exista
                        clave = usuario['clave'].values[0]
                        if clave == actual_password:  # Verificamos que la contraseña sea la del usuario
                            if nueva_password == confirmar_password:
                                dfusuarios.loc[dfusuarios['usuario'] == parUsuario, 'clave'] = nueva_password
                                ...
                                st.error("Confirme bien la contraseña")
                        else:
